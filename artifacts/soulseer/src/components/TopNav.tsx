...
export function TopNav() {
  ...
  const handleSignOut = () => {
    queryClient.clear();
    logout({
      logoutParams: { returnTo: `${window.location.origin}${basePath || ""}/` },
    });
  };

  const handleSignIn = (signup = false) => {
    loginWithRedirect({
      ...
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  <DropdownMenuItem asChild>
                    <Link href="/sessions" className="cursor-pointer w-full">My Readings</Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
          ...
          {!isAuthenticated && !isLoading && (
            <div className="hidden sm:flex items-center gap-4">
              <button onClick={() => handleSignIn(false)} className="text-foreground hover:text-primary transition-colors font-sans text-sm">Sign In</button>
              <button onClick={() => handleSignIn(true)} className="bg-primary/10 border border-primary/30 text-primary px-4 py-2 rounded text-sm hover:bg-primary/20 transition-all shadow-[0_0_10px_rgba(255,105,180,0.1)]">Join</button>
            </div>
          )}
              ...
              <div className="flex flex-col gap-6 mt-8">
                ...
                {isAuthenticated && me && (
                  <div className="flex flex-col gap-4 font-sans">
                    ...
                    <Link href="/sessions" className="text-foreground hover:text-primary">My Readings</Link>
                    {me.role === "reader" && (
                ...
                {!isAuthenticated && !isLoading && (
                  <div className="flex flex-col gap-4 font-sans">
                    <button onClick={() => handleSignIn(false)} className="text-left text-foreground hover:text-primary">Sign In</button>
                    <button onClick={() => handleSignIn(true)} className="text-left text-primary">Join Community</button>
                  </div>
                )}
              </div>
            </SheetContent>
